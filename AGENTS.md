...
`/api/login, /login, /, /index.html, /api/ws, /api/events, /api/lanPeers, /api/heartBeat, /api/authorImage/, /css/, /js/, /assets/, /icons/, /favicon.ico`
...
`UseApp().search.Page(param)` → `pageAsync` 内 `atomic.Value` 无锁读索引 → LRU 缓存(epoch 校验) → bucket 并发搜索 → channel 合并 → 排序分页。
...
- 两者 Hub 均使用 `atomic.Bool hubRunning` 防递归启动

## 前端约定

- Quasar v2 + Vite，Vue Router hash 模式（Electron history），已启用 Notify/AppFullscreen/Dialog
- Pinia + `pinia-plugin-persist`，store 文件 `frontend/src/stores/index.ts`、`System.ts`
- 路径别名 `@` → `frontend/src`；ESLint + Prettier（单引号、分号）；TS 4.5+
- 前端 API 基础地址 `http://localhost:10081`，文件流指向 `:10082`
- `composables/`：`useSSE`、`useChatWs`、`useTorrentDownload`、`useVideoConference` 等
