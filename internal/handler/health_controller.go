...
func GetIndexHealthCheck(c *gin.Context) {
	...
	health.BucketCount = UseApp().search.BucketCount()
	health.IndexNumber = service.IndexNumber.Load()
	health.FullScanInProgress = service.FullScanInProgress.Load()
	health.ExpectedDirs = len(UseApp().config.Get().Dirs)
	health.TotalCount = UseApp().search.GetTotalCount()
	health.TotalSize = UseApp().search.GetTotalSize()
	health.TotalSizeStr = utils.GetSizeStr(health.TotalSize)
	health.LastScanTime = service.GetLastScanTime().Format("2006-01-02 15:04:05")
	health.AuthorCount = UseApp().search.GetAuthorCount()
	health.TagCount = len(UseApp().search.GetTagMenu())
	health.TypeCount = len(UseApp().search.GetTypeMenu()) - 1 // 排除"全部"
	if health.TypeCount < 0 {
		health.TypeCount = 0
	}
	health.SeriesCount = len(UseApp().search.GetSeriesCount())
